...
- **완전 커스텀**: 캐릭터(이모지/내 이미지 파일·크기), 말풍선 문구 목록, 소리(내장 꽥/합성음/내 사운드 파일·볼륨), 표시 시간까지 설정창에서 변경.
- **트레이 상주**: 트레이 아이콘 클릭으로 꽥 테스트 / 설정 / 종료.

> 기본 "꽥" 소리는 상업이용 가능한 실제 오리 녹음(CC BY 3.0, 출처는 [CREDITS.md](CREDITS.md))이 내장돼 있다. 기본 캐릭터는 아직 🦆 이모지 **플레이스홀더**이며, 설정에서 원하는 이미지·사운드로 바꿀 수 있다.
...
- 내장 꽥 소리: "Mallard Duck Quacking" by **Mike Koenig** ([SoundBible](https://soundbible.com/), CC BY 3.0). 전체 목록은 [CREDITS.md](CREDITS.md).

## 📄 라이선스

코드는 MIT. 번들된 서드파티 애셋의 라이선스는 [CREDITS.md](CREDITS.md)를 따른다(재배포·판매 시 저작자 표기 유지 필요).
